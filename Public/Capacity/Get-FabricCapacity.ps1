...
function Get-FabricCapacity {
    ...
    try {
        ...
        if ($capacityId -and $capacityName) {
         ...
         # Step 3: Loop to retrieve all capacities with continuation token
         ...
         do {
             ...
             # Step 7: Add capacities to the list
             if ($null -ne $response) {
                 
                 Write-Message -Message "Adding capacities to the list" -Level Debug
