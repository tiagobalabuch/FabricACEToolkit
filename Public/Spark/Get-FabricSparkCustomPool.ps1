<#
.SYNOPSIS
    Retrieves Spark custom pool from a specified workspace.

.DESCRIPTION
    This function retrieves all Spark custom pool from a specified workspace using the provided WorkspaceId.
    It handles token validation, constructs the API URL, makes the API request, and processes the response.

.PARAMETER WorkspaceId
    The ID of the workspace from which to retrieve Spark custom pool. This parameter is mandatory.

.EXAMPLE
    PS C:\> Get-FabricSparkCustomPool -WorkspaceId "12345"
    This example retrieves all Spark custom pool from the workspace with ID "12345".
...
function Get-FabricSparkCustomPool {
    ...
    try {
        ...
        elseif ($SparkCustomPoolName) {
            $SparkCustomPools | Where-Object { $_.DisplayName -eq $SparkCustomPoolName }
        }
        else {
