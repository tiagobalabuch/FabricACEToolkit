...
.PARAMETER MirroredDatabaseFormat
Specifies the format of the MirroredDatabase definition. Currently, only 'ipynb' is supported.
Default: 'ipynb'.

.EXAMPLE
Get-FabricMirroredDatabaseDefinition -WorkspaceId "12345" -MirroredDatabaseId "67890"

Retrieves the definition of the MirroredDatabase with ID `67890` from the workspace with ID `12345` in the `ipynb` format.

.EXAMPLE
Get-FabricMirroredDatabaseDefinition -WorkspaceId "12345"

Retrieves the definitions of all MirroredDatabases in the workspace with ID `12345` in the `ipynb` format.
...
function Get-FabricMirroredDatabaseDefinition {
    ...
    try {
        ...
        if ($MirroredDatabaseFormat) {
            $apiEndpointUrl = "{0}?format={1}" -f $apiEndpointUrl, $MirroredDatabaseFormat
        }
